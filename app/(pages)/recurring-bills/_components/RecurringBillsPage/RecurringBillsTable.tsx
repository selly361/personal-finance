          ...
          {bills.map((bill) => {
            ...
            return (
              <tr key={bill.id} className="border-b last:border-none">
                ...
                <td
                  style={{ color: bill.status === "paid" ? "#277C78" : "#696868" }}
                  className="py-3 px-4 text-sm text-grey-500 flex items-center gap-2"
                >
                  {bill.due_date_label} {Icon}
                </td>
                <td
                  style={{ color: bill.status === "due_soon" ? "#C94736" : "#201F24" }}
                  className={`py-3 px-4 text-sm text-right font-bold ${amountClass}`}
                >
                  £{millify(bill.amount, { precision: 3 })}
                </td>
              </tr>
            )
          })}
